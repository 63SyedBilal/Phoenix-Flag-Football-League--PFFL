...
class NotificationApiService {
  ...
  static Future<List<NotificationModel>> getAllNotifications() async {
    try {
      ...
      final authToken = dio.options.headers['Authorization']?.toString().replaceAll('Bearer ', '') ?? 'No token';
      print('📬 [NOTIFICATION API] Fetching notifications from: ${dio.options.baseUrl}/notification/all');
      print('📬 [NOTIFICATION API] Auth token present: ${authToken.isNotEmpty && authToken != 'No token'}');
      print('📬 [NOTIFICATION API] Token preview: ${authToken.length > 20 ? authToken.substring(0, 20) + "..." : authToken}');
      ...
      if (response.statusCode == 200) {
        ...
        if (data is Map && data.containsKey('data')) {
          ...
          print('📬 [NOTIFICATION API] Found ${rawList.length} notifications in data field');
          
          // Log all keys in the response map
          print('📬 [NOTIFICATION API] Response map keys: ${data.keys.toList()}');
        } else if (data is List) {
          rawList = data;
          print('📬 [NOTIFICATION API] Found ${rawList.length} notifications in list');
        } else {
          print('⚠️ [NOTIFICATION API] Unexpected response format: ${data.runtimeType}');
          print('⚠️ [NOTIFICATION API] Response content: $data');
        }
        
        // If rawList is empty, log this for debugging
        if (rawList.isEmpty) {
          print('⚠️ [NOTIFICATION API] No notifications found in response. This could mean:');
        ...
        print('✅ [NOTIFICATION API] Successfully parsed ${notifications.length} notifications');
        return notifications;
      }
      
      print('⚠️ [NOTIFICATION API] Unexpected status code: ${response.statusCode}');
      return [];
    } on DioException catch (e) {
      print('❌ [NOTIFICATION API] DioException: ${e.message}');
      if (e.response != null) {
        print('❌ [NOTIFICATION API] Response data: ${e.response?.data}');
        print('❌ [NOTIFICATION API] Response status: ${e.response?.statusCode}');
      }
      rethrow;
    } catch (e) {
