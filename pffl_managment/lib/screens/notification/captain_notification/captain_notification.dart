  ...
  Future<void> _handleAccept(
    ...
    try {
      ...
      if (success && context.mounted) {
        // Trigger notification for the sender (Admin/League Manager)
        if (notification.senderId != null) {
          final auth = Provider.of<AuthProvider>(context, listen: false);
          // Using refereeAcceptedInvite as a generic "Accepted" for now,
          // or we can add a specific one later. For Captain, it's usually "Team Accepted League Invite"
          NotificationSenderService.sendNotification(
            receiverId: notification.senderId!,
            type: 'INVITATION_ACCEPTED',
            message:
                'Captain ${auth.userName} has accepted the invitation for team ${notification.team ?? 'their team'} to join ${notification.league ?? 'the league'}',
          ).catchError((e) => false);
        }
      ...
      if (success && context.mounted) {
        ...
        if (notification.senderId != null) {
          ...
          NotificationSenderService.sendNotification(
            ...
            message:
                'Captain ${auth.userName} has rejected the invitation for team ${notification.team ?? 'their team'} to join ${notification.league ?? 'the league'}',
          ).catchError((e) => false);
        }
